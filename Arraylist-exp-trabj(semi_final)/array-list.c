...
int al_clear(ArrayList* pList)
{
    ...
    if(pList!= NULL)
    {
       auxList=(void**)realloc(pList->pElements,sizeof(void*)* AL_INITIAL_VALUE );
       if(auxList!=NULL)
       {
           pList->pElements=auxList;
           pList->size=0;
           pList->reservedSize = AL_INITIAL_VALUE;
           returnAux=0;
       }
...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    if( pList != NULL  )
    {
        ...
        if(returnAux != NULL)
        {
        returnAux->pElements=(void**)realloc(returnAux->pElements,sizeof(void*)* pList->reservedSize );
 ...
 *                  - ( 0) if Ok
 */
int al_push(ArrayList* pList, int index, void* pElement)//ver 80%
{
    int returnAux = -1;
     if(pList != NULL && pElement != NULL  )
    {
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
   ...
   // int j=0;
    ...
    if(pList != NULL )
    {
      if( (from>=0 && from<=to) && (to>=from && to<=pList->size) )
        {
            returnAux = al_newArrayList();
            if(returnAux != NULL)
            {
             returnAux->pElements=(void**)realloc(returnAux->pElements,sizeof(void*)* (to - from) );
              if(returnAux->pElements != NULL)
                {
                    for(i=from ; i<to ; i++)
                    {
                        al_add(returnAux,pList->pElements[i]);
                        //returnAux->pElements[j] = pList->pElements[i];
                        //j++;
                    }
...
int resizeUp(ArrayList* pList)
{
    ...
    if(pList!= NULL)
    {
       auxList=(void**)realloc(pList->pElements,sizeof(void*)*(pList->reservedSize + AL_INCREMENT));
       if(auxList!=NULL)
       {
           pList->pElements=auxList;
